  ...
  return (
    <section id={id} ref={ref} className={reversed ? 'bg-slate-50' : 'bg-white'}>
      <Container className="py-20 lg:py-28">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="mb-12 max-w-3xl">
          ...
          <p className="mt-3 text-xl font-medium text-slate-600">{headline}</p>
          <p className="mt-4 leading-relaxed text-slate-600">{description}</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }} className="mb-12">
          ...
          <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {capabilities.map((item) => (
              <li key={item} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-5">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="grid gap-8 lg:grid-cols-2">
          <div className="rounded-2xl border border-red-100 bg-red-50/50 p-8">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="font-heading text-lg font-bold text-navy">What breaks without us</h3>
            </div>
          ...
          <div className="rounded-2xl border border-green-100 bg-green-50/50 p-8">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="font-heading text-lg font-bold text-navy">What we own</h3>
            </div>
            <ul className="space-y-3">
              {owns.map((item) => (
                <li key={item} className="flex items-start gap-3 text-sm text-slate-700">
